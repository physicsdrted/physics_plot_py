...
# This CSS uses Flexbox to properly constrain and center the plot image.
st.markdown("""
<style>
/* Target the container Streamlit wraps around the image */
[data-testid="stImage"] {
    /* 1. Set a maximum height for the container */
    max-height: 85vh;

    /* 2. Use Flexbox to center the image within the container */
    display: flex;
    justify-content: center;
}

/* Target the actual image element inside the container */
[data-testid="stImage"] img {
    /* 3. The image should not exceed the container's dimensions */
    max-width: 100%;
    max-height: 100%;

    /* 4. Allow the browser to automatically size the image to fit
