...
# ... (Keep these functions exactly as defined in the previous complete code block) ...
...
def numerical_derivative(func, x, params, h=1e-7):
    ...
    try:
        ...
        return deriv
    except Exception as e: st.warning(f"Error during num derivative: {e}. Returning slope=0."); return np.zeros_like(x) if isinstance(x, np.ndarray) else 0
    ...
    if current_file_key != st.session_state.get('processed_file_key', None):
        ...
        try:
            ...
            for col in df.columns:
                ...
                except ValueError: st.error(f"Col '{col}' non-numeric."); all_numeric = False; break
            ...
            ax_initial.legend()
            plt.tight_layout()
            st.pyplot(fig_initial) # Display the initial plot
            # Don't store fig_initial in session state unless needed elsewhere
            # --- <<< End of Initial Plot Display >>> ---

            st.rerun() # Rerun to update UI state (shows plot and fitting controls)

        except Exception as e:
            st.error(f"Error processing file: {e}")
            st.session_state.data_loaded = False; st.session_state.processed_file_key = None
            st.stop()

# --- Show Input Section only if data is loaded ---
if st.session_state.data_loaded:
    # Check if results already exist from a previous run for this data
    # If not, show the fitting controls. If yes, show the results instead.
    if not st.session_state.get('fit_results', None):
        ...
        if fit_button and eq_string_input:
            # Clear previous final fig if starting new fit
            st.session_state.final_fig = None
            with st.spinner("Performing iterative fit... Please wait."):
                try:
                    # --- Get Equation, Create Function, Iterative Fit ---
                    ...
                    # --- Generate Final Plot Figure --- (No table drawing here)
                    fig = plt.figure(figsize=(10, 9.8)); gs = GridSpec(2, 1, height_ratios=[3, 1], hspace=0.08) # Only 2 rows needed for plots
                    ...
                    fig.tight_layout(pad=1.0) # Add tight layout for plot spacing
                    st.session_state.final_fig = fig # Store figure

                    # Rerun to display results immediately after fit calculation
                    st.rerun()
...
# --- Display Results Section (if results exist) ---
# This section now runs after the fit is done and st.rerun is called
if st.session_state.get('fit_results', None):
    ...
    if st.session_state.final_fig:
        ...
        st.session_state.final_fig.savefig(img_buffer, format='svg', bbox_inches='tight', pad_inches=0.1) # Keep tight layout for saving plot
        img_buffer.seek(0)
